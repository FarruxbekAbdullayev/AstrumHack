...
import { DELETE_USER, FETCH_TRAINERS, FETCH_USER } from '../../services/users.service';
...
const AddModal = lazy(() => import('../../components/Member/MemberAdd'));
const EditModal = lazy(() => import('../../components/Member/MemberEdit'));
...
export default function Members() {
  ...
  let columns = [
    ...
    },
    {
      title: t('Trainer'),
      dataIndex: 'trainer',
      render: (text) => text ? `${text?.firstName} ${text?.lastName}` : t("The coach is not attached"),
      filters: trainersData.map(item => ({text: `${item.lastName} ${item.firstName}`, value: `${item.lastName} ${item.firstName}`})),
      onFilter: (value, record) => `${record?.trainer?.lastName} ${record?.trainer?.firstName}` === value,
    },
    {
      title: t('Phone'),
      dataIndex: 'phone'
    },
    {
      ...
      render: (membership, record) => {
        const hasmembershipDate = record.membershipStart && record.membershipEnd;
        return (
          <div>
            <div><Link to="/settings">{membership?.title} - {membership?.price}</Link></div>
            <small>{hasmembershipDate && `${moment(record.membershipStart).format('DD/MM/YYYY')} - ${moment(record.membershipEnd).format('DD/MM/YYYY')}`}</small>
          </div>
        )
      }
    },
    ...
    },
    {
      title: t('QRCode'),
      dataIndex: 'qrCode',
      render: (text, record) => <QRCodeComponent value={record._id} size={32}/>
    },
    {
      title: t('Action'),
      dataIndex: 'action',
  ...
  return (
    <StyledEquipment>
      ...
      <Table
        size="small"
        id='membersTable'
        rowClassName={(record) => new Date(record.membershipEnd).getTime() <= new Date().getTime() ? "expired__date-off" : ""}
        columns={columns}
              ...
              <Table.Summary.Row className="fw-600">
                ...
                <Table.Summary.Cell  colSpan={4}>
                  ...
                  <p style={{padding: 10, margin: 0}}>
                    {parseInt(record?.reduce((a, b) => (a) + (b?.membership?.price), 0) ?? 0).toLocaleString("ru")}
                  {/* </Text> */}
                  </p>
                </Table.Summary.Cell>
              </Table.Summary.Row>
