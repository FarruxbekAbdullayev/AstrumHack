...
import {  Tabs, Table, message, DatePicker, Drawer, Button, Checkbox, Space, Input, Popconfirm } from 'antd';
import { createTablePagination, t } from '../../utils';
import PageHeader from '../../components/PageHeader';
import StyledPayment from './Payments.style';
import { DELETE_PAYMENT, FETCH_PAYMENTS, FETCH_STATISTICS, UPDATE_PAYMENT } from '../../services/payments.service';
...
const SalesPage = lazy(() => import('../../components/Sales/Sales'));
...
function Payments() {
  ...
  const [filterStatuses, setFilterStatuses] = useState({
    status: new URLSearchParams(window.location.search).get('tab') || 'membership',
    user: null,
  ...
  const fetchData = async () => {
    ...
    if(account.role === 'reception') {
      ...
      if (status == "pending") {
       ...
       const { data } = await FETCH_PAYMENTS({status, startDate: moment().startOf('day'), endDate: moment().endOf('day')});
        response = data
     }
  ...
  const handlePeriod = (e) => {
    setFilterStatuses((state) => ({...state, startDate: e?.[0]?._d?.toISOString().slice(0, 10), endDate: e?.[1]?._d?.toISOString().slice(0, 10)}))
  } 
  ...
  const dataFilterHandle = async(e) => {
    ...
    const { data } = await FETCH_PAYMENTS({status, startDate: e[0].startOf("day"), endDate: e[1].endOf("day")});
    if (data) {
      setData(data?.payments)
    }
    } 
  ...
  if(account.role === 'reception') {
    const columnsFilter = columns.filter(item => item.dataIndex !== 'originalPrice' && item.dataIndex !== 'totalPrice');
  ...
  return (
    <StyledPayment>
      <Drawer title={t("Filter")} placement="right" onClose={onClose} visible={visible}>
        <RangePicker style={{ width: '100%' }} onChange={(e) => handlePeriod(e)} />
        <Checkbox.Group style={{ margin: '15px 0' }} onChange={handleCheckbox} options={statusOptions} />
        <Space size="middle">
          <Button type="dashed" size="middle" onClick={() => window.location.reload()}>{t("Reset")}</Button>
          <Button type="primary" size="middle" onClick={filterData}>{t("Filter")}</Button>
        </Space>
      </Drawer>
    
      <div className="body">
        <Tabs activeKey={activeTab} type="card" onChange={key => setActiveTab(key)}>
          <TabPane tab={t('Membership')} key="membership">
            ...
            <Tabs activeKey={status} onChange={key => setFilterStatuses({...filterStatuses, status: key})}>
                    ...
                    <Table.Summary.Row className="fw-600">
                      ...
                      <Table.Summary.Cell  colSpan={4}>
                        <p style={{padding: 10, margin: 0}}>
                          {parseInt(record?.reduce((a, b) => (a) + (b?.price), 0) ?? 0).toLocaleString("ru")}
                        </p>
                      </Table.Summary.Cell>
                    </Table.Summary.Row>
                  </>
          ...
          <TabPane tab={t('Products')} key="products">
            {/* {isAdmin && <StatisticsItem statistics={statistics.sales}/>} */}
            <SalesPage status={status}/>
          </TabPane>
        </Tabs>
      </div>
      {show ? (
